...
trait Expr {
  def derive(v: Variable): Expr = this match {
    ...
    case FuncCall(f, args) => Product(FuncCall(f, args), f.derive(v))
    case ImplicitFunc1(name, e) => Derivative(this, v)
  }

  def substitute(subs: Map[Expr, Expr]): Expr = {
    this match {
      case v: Variable => subs.getOrElse(v, v)
      case Number(value) => Number(value)
      case Sum(e1, e2) => Sum(e1.substitute(subs), e2.substitute(subs))
      case Product(e1, e2) => Product(e1.substitute(subs), e2.substitute(subs))
      case Power(base, exp) => Power(base.substitute(subs), exp.substitute(subs))
      case FuncCall(f, args) => FuncCall(f, args.map(_.substitute(subs)))
      case ImplicitFunc1(name, e) => ImplicitFunc1(name, e.substitute(subs))
      case _ => this
    }
  }
...
case class Assignment2(name0: String, name1: String, expr: Expr) extends Expr with Assignment {
  ...
  override def visit(env: Option[Environment]) = ErrorExpr("Todo Assignment2.visit")
}
...
case class Assignment3(name0: String, name1: String, name2: String, expr: Expr) extends Expr with Assignment {
  ...
  override def visit(env: Option[Environment]) = ErrorExpr("Todo Assignment3.visit")
}
